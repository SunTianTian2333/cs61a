...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    "*** YOUR CODE HERE ***"
    if num_rolls==0:
       sum=hefty_hogs(player_score,opponent_score)
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while max(score0,score1)<goal:
          ...
          if who==0: 
             temp=strategy0(score0,score1)
             score0+=take_turn(temp, score0, score1, dice, goal)
             score0+=hog_pile(score0,score1)
          else: 
             temp=strategy1(score1,score0)
             score1+=take_turn(temp, score1, score0, dice, goal) 
...
def hefty_hogs_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    sum=hefty_hogs(score,opponent_score)
    if sum>=threshold:
       return 0
    else: 
       return num_rolls  
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    if hog_pile(score,opponent) or hog_pile(score,opponent)+score >=threshold:
       return 0
